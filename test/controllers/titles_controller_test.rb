...
class TitlesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create title" do
    assert_difference("Title.count") do
      post titles_url, params: { title: { movie_character_id: @title.movie_character_id, name: @title.name } }
    end
  ...
  test "should update title" do
    patch title_url(@title), params: { title: { movie_character_id: @title.movie_character_id, name: @title.name } }
